...
#+begin_src c :noweb-ref lock-confirm
  void lock_confirm(uint8_t cmd, uint8_t board, uint8_t * data, uint8_t len) {
    ...
    case LOCK_CMD_STATUS: {
      ...
      if (board < MAX_BOARD) {
        engineer_state = engineer_transform_state(engineer_state, ENGINEER_LOCK_STATUS_ACK_COMMA_BOARD_LESS_THAN_MAX_EVENT, tmp);
      } else {
        engineer_state = engineer_transform_state(engineer_state, ENGINEER_LOCK_STATUS_ACK_COMMA_BOARD_EQUALS_MAX_EVENT, tmp);
      }
      break;
    }
    default:
