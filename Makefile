...
CORESRC:=$(BUILDDIR)/boxos.c $(BUILDDIR)/parameter.c $(BUILDDIR)/packet.c
CHARGERSRC:=$(BUILDDIR)/charger.c
DRIVERSRC:=$(BUILDDIR)/uart.c $(BUILDDIR)/uart3.c $(BUILDDIR)/uart3-packet.c $(BUILDDIR)/uart4.c $(BUILDDIR)/uart4-packet.c $(BUILDDIR)/led.c $(BUILDDIR)/light.c $(BUILDDIR)/ultraviolet.c $(BUILDDIR)/camera.c $(BUILDDIR)/fan.c
EGGSRC:=$(BUILDDIR)/egg.c $(BUILDDIR)/egg_payload.c
...
RFIDSRC:=$(BUILDDIR)/rfid-reader.c $(BUILDDIR)/rfid_payload.c
...
RFIDFSMSRC:=$(BUILDDIR)/rfid-reader-fsm.c
...
$(UTILITYSRC): utility.org | prebuild
	org-tangle $<
$(RFIDSRC): rfid-reader.org | prebuild
	org-tangle $<
...
#	sed -i 's/printf/console_string/g' $@
#	sed -i 's/\\n/\\r\\n/g' $@
$(RFIDFSMSRC): rfid-reader-fsm.txt | prebuild
	fsm-generator.py $< -d $(BUILDDIR) --prefix rfid_reader --style table
#	fsm-generator.py $< -d $(BUILDDIR) --prefix rfid_reader --style table --debug
#	sed -i '1a#include "console.h"' $@
#	sed -i '1d' $@
#	sed -i 's/printf(\"(\");/console_log(\"(\");/g' $@
#	sed -i 's/printf/console_string/g' $@
